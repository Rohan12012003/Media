...
export async function getUserPosts (userId?:string){
    if(!userId) return;
    try {
        let postsByUser = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.postCollectionId,
            [Query.equal("creator",userId),Query.orderDesc("$createdAt")],
        )

        if(!postsByUser) throw Error;
...
export async function getFollower(id: string, currentUserId: string) {
    try {
        // Get the target user
...
export async function removeFollower(id: string, currentUserId: string) {
    try {
        // Get the target user
        const targetUser = await databases.getDocument(
            appwriteConfig.databaseId,
        ...
        if (targetUser) {
            // Update the target user's followers
            const updatedFollowers = targetUser.followers.filter(
            ...
            if (currentUser) {
                ...
                const updatedFollowing = currentUser.following.filter(
                    (followedUserId: string) => followedUserId !== id
                );
...
export async function getLikedPosts(id: string) {
    try {
        // Fetch posts with the latest 20 posts
        const posts = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.postCollectionId,
            [Query.orderDesc('$createdAt'), Query.limit(20)]
        ...
        // Filter posts based on whether the user with the given id liked each post
        let result: Models.Document[] =[];
        const likedPosts = posts.documents.filter((post) => {
            const likesArray:Models.Document[] = post.likes;
            const array = likesArray.map((like) => {
                if(like.$id === id)
                result.push(post);
            });
